...
for i,n in enumerate(l.ns):
    ...
    Rs_ins[:,i] = prs.Rs
    Rs_oos[:,i] = [R_star(q=p.q,M_square=p.M_square) for p in prs.ps]
    CEs_ins[:,i] = prs.CEs
    CEs_oos[:,i] = [CE_star(q=p.q,M_square=p.M_square) for p in prs.ps]
...
saver(datas,l,'Student-Nvarying-Saturated')
