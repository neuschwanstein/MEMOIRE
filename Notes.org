...
<<definitionCost>>
			...
			c(p,r) = U(r) - U(pr + (1-p)R_f) if r > r_c
			c(p,r) = U(0) - U(pr + (1-p)R_f) if r \leq r_c
				    p = q_S^{T}x = q(S)^{T}x
...
These results follow from the [[sigmaAdmissibilityTheorem][\sigma-admissibility theorem]] of our [[lossFunctionDefinition][loss function]] and
[[BousquetTheorem7][Bousquet's Theorem]]. \diamond
