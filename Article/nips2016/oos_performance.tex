\subsection{Out-of-sample performance bounds}
...
\begin{thm}\label{thm:outsampleBound1}
  ...
  %\[    \CE(\qhat;\F) \geq \CE(\qhat;\Fhat) - u_{-1}'(\CE(\qhat;\Fhat))\\frac{(\gamme^2\bar{r}\xi)^2}{2\lambda} \left(\frac{1}{n} + \frac{4\sqrt{\log(1/\delta)}}{\sqrt{2n}}\right),   \]
  \[ 
...
$\qhat$ decision is absolutely bounded \Erick{What does this mean in words?} with
